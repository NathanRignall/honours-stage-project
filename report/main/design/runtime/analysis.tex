...
% The paper results
\textcite{abeniUsingXenKVM2020} compiled a similar study into the real time performance of open source hypervisors. Every hypervisor introduces some latencies into a virtual machine that can affect the correctness of the existing real-time analysis. Abeni and Faggioli compared the performance of both KVM and Xen on various intel x86 processors.
...
This problem as connotations to the issues \textcite{abeniUsingXenKVM2020} experienced with timer resolutions negatively affecting latency evaluations. As cyclictest uses the local cpu timer to measure latencies it is important that it is at a high enough resolution.
...
Xen approximately doubled the interrupt latency measured as compared to bare metal Linux. This is not ideal, but Xen could still be used in a soft-real time application if the latencies were accounted for. Before deploying Xen in a real-time environment, additional test should evaluate the multi-core interference across multiple guest virtual machines.
